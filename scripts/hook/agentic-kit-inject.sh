...
cat <<'EOF'
[agentic-kit · Swift/iOS] Always on (internal, no external deps): swift-dev (domain) · task-execution (deliver). Optional: ponytail.
...
Priority chain (first tier with an answer wins): project (MEMORY.md+code) -> knowledge packs, curated (akios/code-references/ = code pack) -> ~/.claude/akios/preferences.md -> baseline packs (swift-dev = ios pack).
...
Single source: spec state lives only in akios/Roadmap.md -> never mirror the ## Specs table into CLAUDE.md.
Modes: brainstorm/plan -> plan mode (review before edits); deliver -> accept-edits/auto (Shift+Tab cycles).
...
(verify+review+fix loop; park red, never ship broken).
...
- Full feature (idea to code) -> ios-feature-pipeline (reads akios/workflow.yml: brainstorm -> plan -> design -> deliver)
- Idea -> spec           -> idea-to-spec (/akios:brainstorm; write to akios/specs/, register in akios/Roadmap.md)
- Spec -> tasks          -> spec-to-tasks (/akios:plan; writes akios/tasks/todo/)
...
- Executing the backlog  -> task-execution (/akios:deliver)
- Before "done"          -> /verify + /code-review
Read AGENTS.md -> akios/Context.md if not already loaded. Durable project decisions live in
