...
2. **The positions** (up to 3 / 2–3; never pad to a fourth), each with: a short name, what it means concretely, and its honest cost. The recommendation is visibly marked (e.g. "recommended") and carries its reason inline — not in a separate paragraph the user has to hunt for.
3. **The open path** — a first-class line for the user to answer in their own words ("…or write your own"), shown alongside the options as a peer, never a buried footnote. Picking and writing your own are equally available.
...
1. **Frame** as usual.
2. **Grounded facts** — if external facts would move the answer, the searched-and-cited findings come first, before the positions (per "Grounding moves" below).
3. **The positions** (still up to 3), but each now carries its **second-order consequences**: what it forecloses, reversible vs one-way, who it helps/hurts downstream — not just the one-line cost.
