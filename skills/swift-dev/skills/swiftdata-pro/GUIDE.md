...
1. If the project layers a Store/Repository or Active Record pattern over SwiftData, follow the project's preferred architecture in `references/store-repository-pattern.md`.
...
- `references/store-repository-pattern.md` - preferred data-layer architecture: PersistenceStack, generic Repository protocol + CRUD extension, StoreRepository (DI), and the Active Record pattern (DataBase singleton + ActiveRecord protocol).
