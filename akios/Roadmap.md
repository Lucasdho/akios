...
| ui-first-architecture.md | UI-first ordering + structure: per-feature folders, layer split, build-order law, dumb-component `init` rule, factory/router DI | done | UI overhaul, spec 2 of 3 (C→A→B) — §1/§2 folder shape SUPERSEDED (confirmed 2026-07-01) by ALVA §4/§6, see alva-adoption.md D1/D2; §3–§8 behavioral laws realized in `skills/swift-dev/skills/alva-architecture/GUIDE.md` (build-order, dumb-component rule, Router/Container DI all present) |
...
| akios-backlog-map.md | AKIOS backlog synthesis (index — routes the whole family) | done | read FIRST; maps every backlog item B1–B37 → covering spec; its own §5 recommended build order has fully shipped (all G1–G11,G13 specs done as of v0.8.1) — only G12/G14 (parallel-execution-scheduling.md, subagent-context-chaining.md) remain by design, both explicitly non-critical-path |
...
| subagent-context-chaining.md | Subagent task-batch chaining — one subagent works a sequential batch of tasks, self-compacting between them; at a 120k-token lineage budget it hands off in writing and the orchestrator spawns a fresh subagent to continue, so neither side's context grows unbounded | planned | backlog B38; answers the `feedback_subagent_dispatch` memory's ~300k-token subagent incident with a repeatable threshold instead of an ad hoc judgment call; orthogonal to parallel-execution-scheduling.md (depth vs. breadth); T074-T076 in `akios/tasks/todo/` wire the two `[CONSEQUENCE]` doc edits |
