...
| subagent-context-chaining.md | Subagent task-batch chaining — one subagent works a sequential batch of tasks, self-compacting between them; at a 120k-token lineage budget it hands off in writing and the orchestrator spawns a fresh subagent to continue, so neither side's context grows unbounded | done | backlog B38; answers the `feedback_subagent_dispatch` memory's ~300k-token subagent incident with a repeatable threshold instead of an ad hoc judgment call; orthogonal to parallel-execution-scheduling.md (depth vs. breadth); realized via T074-T076 (0.8.2) |
