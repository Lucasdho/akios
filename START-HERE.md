...
**Or — paste this (no plugin).** Prefer the script route over the plugin? Open Claude
...
1. Get the kit. Clone it to ~/akios if it isn't there yet:
     git clone https://github.com/Lucasdho/akios.git ~/akios
   If the folder already exists, run `git -C ~/akios pull` to update it.
2. Install the helper skills: ~/akios/scripts/install-skills.sh
   There are NO required external plugins — the kit ships everything it routes to
   (swift-dev, idea-to-spec, spec-to-tasks, task-execution, oss-first).
...
- **swift-dev** — a library of Apple/Swift expertise the assistant pulls in when it touches
