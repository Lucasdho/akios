...
## Collaboration
<!-- Written by /akios:setup. One of: solo | team.
...
## Autonomy
<!-- Written by /akios:setup (default: manual); overridable per run via a command flag
     ...
     manual → just-vibes never pushes/merges/opens a PR, even under --force; a green unit's
              branch stays local and is reported in the run's "Built (unshipped)" bucket,
              awaiting a human push/merge. Team-mode claim-coordination pushes are unaffected —
              only the shipping action is withheld.
