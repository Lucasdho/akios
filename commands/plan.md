...
# /akios:plan — Spec-to-tasks (akios/workflow.yml: plan)

**Guard (soft).** Confirm the repo is initialized and the `plan` phase's `prereqs` from
`akios/workflow.yml` exist (`akios/specs/*.md`). If no spec exists, **don't hard-block** — say so and **offer**
...
`akios/Roadmap.md`. No `.specify/`, no constitution, no speckit.
