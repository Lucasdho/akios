---
description: Autonomous run — akios drives the whole pipeline itself (brainstorm→plan→execute→deliver) on the next fuel, stopping at the spec boundary by default.
...
`.claude/.agentic-kit-version`). If not, **don't hard-block** — offer `/akios:init` first.
...
- **Claim** the unit (signature claim record; commit + push in team mode), then run only the phases
  needed to reach shippable (unattended **deepthink** brainstorm if there's no spec → plan → execute).
