...
- **No auto-run of interactive phases** (Decision 2-C declined) — *revisited in §9: the `just-vibes`
  run-style auto-runs the interactive phases unattended, as an explicit opt-in.*
- **No per-task push/merge** — the only human gate is at the end, before push/merge — *revisited in
  §9: `just-vibes` waives the human push/merge gate (not the quality gate).*

---

## 9. Autonomous run + multi-instance  *(v0.7.0)*

**`just-vibes` run-style** — drives the spine unattended (see `skills/just-vibes/SKILL.md`):
- **Fuel precedence:** explicit idea → `tasks/todo/` → `specs/*.md @ designed` → `Vision.md` /
  `Roadmap.md` item (the last needs unattended **deepthink** brainstorm).
- **Default** = one unit then stop at the spec boundary; **`--force`** = loop until fuel exhausted /
  interrupted.
- **Waives** the human push/merge gate (the invocation is the authorization); **keeps** the quality
  gate — verify + code-review + a bounded fix loop (stop after two no-progress cycles), then **park**
  a still-red spec (branch + logs, `blocked`), never deliver broken.
- **Delivery** follows `Roadmap.md` `collaboration`: solo → merge + push default branch; team → push
  `feature/<spec>` + open a PR.

**Multi-instance (team mode)** — git-based, safety-first, no central server:
- **Instance signature** (`scripts/akios-instance.sh` → `user@host/id`) on commit trailers + claims
  lets a teammate's akios recognize others' work.
- **Claim-before-work:** ownership recorded in **committed** files (task frontmatter `owner:` / the
  `Roadmap.md` line); `git pull` then claim + push; **push-rejection is the lock**; yield if taken.
- **Roadmap.md stays single-source** with a **monotonic-status merge** (higher status wins; edit only
  your line, never reorder) so an unattended run resolves conflicts without a human.
- **Declined:** a central assignment server / lead-orchestrator (too much machinery; the user chose
  safety-first). Shared-checkout filesystem locking is secondary to the git-based model.
